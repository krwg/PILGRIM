...
import { chapterHash, startHashRouter } from './router/hash.js';
import { DEFAULT_STORAGE_KEYS, loadAllProgress, loadSettings, saveChapterProgress, saveSettings, } from './storage/progress.js';
import { adjacentReadableIds, isChapterReadable, } from './manifest/access.js';
import { defaultChapterNavHtml, defaultLockedChapterHtml, defaultRenderChapterHtml, defaultSlots, } from './slots/defaults.js';
import { resolveReaderFeatures, resolveReaderStrings, } from './i18n/strings.js';
import { mountReaderChrome } from './chrome/toolbar.js';
import { bindFigureLightbox } from './media/lightbox.js';
import { bindChapterGestures, showContinuePrompt, } from './navigation/continue.js';
...
function chapterFileUrl(baseUrl, file) {
...
export async function createReader(options) {
    ...
    const slots = { ...defaultSlots, ...options.slots };
    ...
    const chapterAccess = options.chapterAccess ?? 'published-only';
    ...
    let settings = loadSettings(storageKeys);
    if (options.initialThemeName)
        settings.theme = options.initialThemeName;
    if (!themes[settings.theme])
        settings.theme = Object.keys(themes)[0] ?? dossierTheme.name;
    let manifest = null;
    ...
    let chrome = null;
    function themeColorForSettings() {
    ...
    async function renderLocked(entry) {
        clearChapterRuntime();
        document.body.classList.remove('ps-in-reader', 'in-reader');
        chrome?.destroy();
        chrome = null;
    ...
    async function renderChapter(chapterId) {
        ...
        const renderOpts = {
            ...options.render,
            renderExhibit: (inner, glossary, opts) => slots.DocumentExhibit({
                inner,
                glossary,
                options: opts,
            }),
        };
        const { prevId, nextId, nextEntry } = adjacentReadableIds(m.chapters, chapterId, chapterAccess);
        ...
        const onScroll = () => {
            ...
            window.clearTimeout(onScroll.t);
            onScroll.t = window.setTimeout(() => {
    ...
    async function onRoute(route) {
        ...
        if (route.kind === 'chapter')
            await renderChapter(route.chapterId);
        else
            renderHome(m);
    }
...
//# sourceMappingURL=createReader.js.map