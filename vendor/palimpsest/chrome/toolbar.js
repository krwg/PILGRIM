...
function themeIcon(themes, name) {
    return themes[name]?.icon || '◐';
}
function spacingValue(spacing) {
    return { compact: 1.55, normal: 1.75, spacious: 2 }[spacing] || 1.75;
}
export function mountReaderChrome(options) {
    const { settings, storageKeys, strings, themes, onSettingsChange, } = options;
    const themeOrder = options.themeOrder ??
    ...
    function apply() {
        ...
        const prose = document.querySelector('.reader .prose, .ps-prose');
        if (prose) {
        ...
        if (themeBtn)
            themeBtn.textContent = themeIcon(themes, settings.theme);
        const narrowBtn = document.getElementById('ps-toggle-narrow');
        if (narrowBtn)
            narrowBtn.classList.toggle('active', settings.narrow);
        const chromeBtn = document.getElementById('ps-toggle-chrome');
        if (chromeBtn)
            chromeBtn.classList.toggle('active', settings.chromeHidden);
        const sizeLabel = document.querySelector('.reader-size-label');
        if (sizeLabel)
            sizeLabel.textContent = `${Math.round(settings.size * 100)}%`;
    }
    ...
    document.querySelectorAll('[data-spacing]').forEach((btn) => {
        btn.onclick = () => {
            settings.spacing = (btn.dataset.spacing || 'normal');
            persist();
            apply();
            closePopover();
        };
    ...
    document.getElementById('ps-reader-reveal').onclick = () => {
        ...
        chromeHideTimer = window.setTimeout(() => {
            if (settings.chromeHidden)
                document.body.classList.add('chrome-hidden');
        }, 4000);
    };
    const onDocClick = (e) => {
        const t = e.target;
        if (popoverOpen && !t.closest?.('#ps-reader-bar'))
            closePopover();
    };
