export function parseMetaBlock(fmBlock) {
    const meta = {};
    let key = null;
    const bag = {};
...
export function extractGlossary(bodyRaw) {
    const glossary = {};
    ...
    for (const chunk of chunks) {
        ...
        if (m)
            glossary[m[1]] = m[2].trim();
        else
            bodyParts.push(chunk);
    }
    return { glossary, body: bodyParts.join('').trim() };
}
export function parseChapter(raw, fallback = {}) {
