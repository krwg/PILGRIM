...
export function defaultChapterTransition(ctx) {
    ctx.root.innerHTML = ctx.html;
}
export function defaultChapterNavHtml(ctx) {
    const policy = ctx.chapterAccess ?? 'published-only';
    const strings = { ...defaultReaderStrings, ...ctx.strings };
...
export function defaultLockedChapterHtml(entry, strings = defaultReaderStrings) {
...
export function defaultRenderChapterHtml(parsed, options = {}, extras = {}) {
