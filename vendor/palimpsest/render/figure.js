...
export function parseFigureInner(inner) {
...
export function safeFigureSrc(src, prefix = 'assets/img/') {
    const s = src.trim();
    const escaped = prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const re = new RegExp(`^${escaped}[a-zA-Z0-9._/-]+\\.(png|jpe?g|webp|svg)$`, 'i');
    if (!re.test(s))
        return null;
    return s;
}
export function renderFigure(inner, options = {}) {
    ...
    const credit = options.figureCredit === undefined
        ? ''
